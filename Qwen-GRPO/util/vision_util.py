...
import os
# By default this script has the deafult directory set as the current directory wherever it is placed.
# Since, the AOKVQA images are placed in a different directory, so changing the default directory of this script is the only option.(to root)

# Set the desired working directory
os.chdir("/home/aritrad/")
...
def fetch_image(ele: dict[str, str | Image.Image], size_factor: int = IMAGE_FACTOR) -> Image.Image:
    ...
    if isinstance(image, Image.Image):
        image_obj = image
        # print('Img object detected')

    elif image.startswith("http://") or image.startswith("https://"):
        image_obj = Image.open(requests.get(image, stream=True).raw)
        # print('Img URL detected')

    elif image.startswith("file://"):
        image_obj = Image.open(image[7:])
        # print('Img File// detected')

    elif image.startswith("data:image"):
        # print('Img data detected')
        data = image.split(";", 1)[1]
        if data.startswith("base64,"):
            data = base64.b64decode(data[7:])
            image_obj = Image.open(BytesIO(data))
    else:
        # print('Default case: Img Path detected:', image, '\n')
